class NocBoatRegistrationsController < ApplicationController
  ...
  def create
    @noc_boat_registration = NocBoatRegistration.new(noc_params)
    ...
    respond_to do |format|
      if @noc_boat_registration.save
        ...
        @noc_boat_registration.update_attributes(:file_no => "NOC2021UON" + @current_sq_number.to_s.rjust(6, "0"))
  ...
  def checkOut
    ...
    @current_sq_number = @file_sequence.sequence_no.to_i + 1
    @payment.tnr_no = "NOC2021PAY" + @current_sq_number.to_s.rjust(6, "0")
    @file_sequence.update_attributes(:sequence_no => @current_sq_number)
    @payment.save

    if @payment.present?
         payment_url = cc_avenue_details(@payment, 'https://boatingcontrolroom.aptdc.in/noc_boat_registrations/pay_zippy_response')
        # payment_url = cc_avenue_details(@payment, 'http://localhost:3000/noc_boat_registrations/pay_zippy_response')
...
private
def noc_params
  params.require(:noc_boat_registration).permit(:aadharCard, :adress, :agency_name, :application_status, :applied_date, :approved_date, :boat_name, :boat_type, :engine_type, :expire_date, :file_no, :manufacturing_certificate, :manufacturing_certificate_number, :manufacturing_date, :mobile_number, :owner_email, :owner_name, :paid_amount, :proposed_location, :proposed_use, :tnr_no, :user_id,:controll_room_id)
end
