class PermissionOrdersController < ApplicationController
  ...
  def index
    ...
    if !current_user.employee
      @permission_orders = PermissionOrder.where(user_id: current_user.id).order("created_at DESC")
    elsif current_user.unit_user || current_user.admin
      @permission_orders = PermissionOrder.where(controll_room_id: current_user.control_room_id).order("created_at DESC")
    else
      @permission_orders = PermissionOrder.all.order("created_at DESC")
    end
