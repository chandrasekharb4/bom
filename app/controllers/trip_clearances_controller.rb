class TripClearancesController < ApplicationController
  ...
  def index
    ...
    if(params[:boat_id] && params[:boat_id]!='')
      @trip_clearances += TripClearance.where("boat_id=?",params[:boat_id]).date_scope(@selectedDate).order("created_at DESC")
      
  else
     
   @boats.each do |boat|
        @trip_clearances += TripClearance.where("boat_id=?",boat.id).date_scope(@selectedDate).order("created_at DESC")
      end
  end
